...
hot-reload. If needed, the script can load data from data files other than the
...
def make_parser():
    ...
    parser = argparse.ArgumentParser(
        description="Explore past and future close approaches of near-Earth objects."  # noqa
    )
    ...
    repl.add_argument(
        '-a', '--aggressive', action='store_true',
         help="If specified, kill the session whenever a project file is modified.")  # noqa
...
def inspect(database, pdes=None, name=None, verbose=False):
    ...
    are no matching NEOs. At least one of `pdes` and `name` must be given.
    If both are given, prefer to look up the NEO by the primary designation.
...
class NEOShell(cmd.Cmd):
    ...
    sessions. It wraps the `inspect` and `query` parsers to parse flags for
    ...
    def __init__(
        self,
        database, inspect_parser, query_parser,
    ...
    def precmd(self, line):
        ...
        if changed:
            print("The following file(s) have been modified since this interactive session began: "  # noqa
                  f"{', '.join(str(f.relative_to(PROJECT_ROOT)) for f in changed)}.",  # noqa
                  file=sys.stderr)
            if not self.aggressive:
                print("To include these changes, please exit and restart this interactive session.",  # noqa
                      file=sys.stderr)
            else:
